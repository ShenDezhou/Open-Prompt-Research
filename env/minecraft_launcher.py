...
class MinecraftInstance:
    def __init__(
        ...
        self.mc_dir = 'minecrafts'#minecraft_launcher_lib.utils.get_minecraft_directory()
        self.port = None
        ...
        self.mc_process = SubprocessMonitor(
            ...
            name="minecraft",
            ready_match=r"\[Server thread/INFO\]: Starting Minecraft server on [0-9]+(?:\.[0-9]+){3}:([0-9]+)",
    ...
    def get_mc_command(self):
        file_path = os.path.abspath(os.path.dirname(__file__))
        work_dir = U.f_join(file_path, self.mc_dir)
        jar_dir = U.f_join(file_path, self.mc_dir, "server1_13_1.jar")
        mc_command = f"cd {work_dir} && java -Xms200m -Xmx1300m -jar {jar_dir} nogui --singleplayer bot".split(' ')
    ...
    def run(self):
        self.mc_process.run()
        pattern = r"\[Server thread/INFO\]: Starting Minecraft server on 127.0.0.1:(\d+)"
